...
export default function ScreenshotCalculatorPage() {
  ...
  const processImage = async () => {
    ...
    setError(null)
    clearDebugLogs()
    addDebugLog('🚀 Starting OCR processing...')
    addDebugLog(`📁 File: ${selectedFile.name} (${(selectedFile.size / 1024).toFixed(1)} KB)`)

    try {
      const formData = new FormData()
      formData.append('image', selectedFile)
      addDebugLog('📦 FormData prepared, sending to API...')

      // Create AbortController for timeout
      const controller = new AbortController()
      const timeoutId = setTimeout(() => {
        addDebugLog('⏰ Request timeout (45s), aborting...')
        controller.abort()
      }, 45000) // 45 second timeout

      addDebugLog('🌐 Sending POST request to /api/parse-ocr')
      ...
      addDebugLog(`📡 Response received in ${duration}ms, status: ${response.status}`)

      if (!response.ok) {
        const errorText = await response.text()
        addDebugLog(`❌ HTTP Error: ${response.status} - ${errorText}`)
      ...
      if (result.success) {
        addDebugLog(`📊 Raw text extracted: ${result.rawText?.length || 0} characters`)
        addDebugLog(`📋 Number of lines: ${result.numberOfLines || 0}`)
        setRawText(result.rawText || '')
        setRawLines(result.rawLines || [])
        
        // Set Column Data if available
        if (result.columnData) {
          addDebugLog('📊 Column data received')
        ...
        if (records.length > 0) {
          addDebugLog(`🧮 Parsed records received: ${records.length}`)
          setAttendanceData(records)
          const grouped = groupSubjectsByCode(records)
          addDebugLog(`📚 Grouped subjects: ${grouped.length}`)
          ...
          addDebugLog('ℹ️ No parsed records returned; showing raw text only')
          setAttendanceData([])
          setGroupedSubjects([])
          setCsvHeaders([])
          setCsvRows([])
        ...
        addDebugLog(`❌ Processing failed: ${result.error || 'Unknown error'}`)
        setError(result.error || 'Failed to process image')
      }
    } catch (err) {
      if (err instanceof Error) {
        if (err.name === 'AbortError') {
          addDebugLog('⏰ Processing timeout - request aborted')
        ...
        addDebugLog('💥 Unknown error occurred')
